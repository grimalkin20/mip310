...
if (isset($_GET['restore']) && is_numeric($_GET['restore'])) {
    $id = (int)$_GET['restore'];
    $sql = "SELECT * FROM contacts_recycle WHERE id = ?";
    ...
    if ($result->num_rows == 1) {
        ...
        if ($restore_stmt->execute()) {
            ...
            $delete_sql = "DELETE FROM contacts_recycle WHERE id = ?";
            $delete_stmt = $conn->prepare($delete_sql);
            $delete_stmt->bind_param("i", $id);

            if ($delete_stmt->execute()) {
                logActivity($_SESSION['user_id'], 'Restore Contact', "Restored contact from: {$contact['name']}");
...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    if ($result->num_rows == 1) {
        ...
        if ($delete_stmt->execute()) {
            logActivity($_SESSION['user_id'], 'Permanent Delete Contact', "Permanently deleted contact from: {$contact['name']}");
...
// Get all recycled contacts
$sql = "SELECT *, DATEDIFF(NOW(), deleted_at) as days_in_bin FROM contacts_recycle ORDER BY deleted_at DESC";
$result = $conn->query($sql);
$recycled_contacts = $result->fetch_all(MYSQLI_ASSOC);

logActivity($_SESSION['user_id'], 'View Recycle Bin', 'Viewed contact recycle bin');
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Recycle Bin - Contacts - Admin Panel</title>
        ...
        <div class="main-content">
            ...
            <div class="content">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h2><i class="fas fa-trash me-2"></i>Recycle Bin - Contacts</h2>
                ...
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-trash me-2"></i>Deleted Contacts</h5>
                        <small class="text-muted">Items are automatically deleted after 10 days</small>
                    </div>
                    <div class="card-body">
                        <?php if (empty($recycled_contacts)): ?>
                            <div class="text-center py-5">
                                <i class="fas fa-trash fa-3x text-muted mb-3"></i>
                                <h5 class="text-muted">Recycle bin is empty</h5>
                                <p class="text-muted">No deleted contacts found.</p>
                        ...
                        <?php else: ?>
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        <?php foreach ($recycled_contacts as $index => $contact): ?>
                                            <tr class="<?php echo $contact['days_in_bin'] >= 7 ? 'table-warning' : ''; ?>">
                                                ...
                                                <td>
                                                    <?php 
                                                    $days_left = 10 - $contact['days_in_bin'];
                                                    if ($days_left <= 0): ?>
                                                ...
                                                <td>
                                                    <small class="text-muted">
                                                        <?php echo formatDate($contact['deleted_at']); ?>
                                                    </small>
                                                </td>
                                                <td>
                                                    <div class="btn-group" role="group">
                                                        <a href="?restore=<?php echo $contact['id']; ?>" 
                                                        ...
                                                        <a href="?delete=<?php echo $contact['id']; ?>" 
                                                           ...
                                                           onclick="return confirm('Are you sure you want to permanently delete this contact? This action cannot be undone.')">
