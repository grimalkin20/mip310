...
if (isset($_GET['restore']) && is_numeric($_GET['restore'])) {
    $id = (int)$_GET['restore'];
    $sql = "SELECT * FROM users_recycle WHERE id = ?";
    ...
    if ($result->num_rows == 1) {
        ...
        $restore_sql = "INSERT INTO users (user_id, username, full_name, email, mobile_number, user_type, status, created_at) VALUES (?, ?, ?, ?, ?, ?, 'active', NOW())";
        $restore_stmt = $conn->prepare($restore_sql);
        $restore_stmt->bind_param("isssss", $user['user_id'], $user['username'], $user['full_name'], $user['email'], $user['mobile_number'], $user['user_type']);

        if ($restore_stmt->execute()) {
            // Delete from recycle bin
            $delete_sql = "DELETE FROM users_recycle WHERE id = ?";
            $delete_stmt = $conn->prepare($delete_sql);
            $delete_stmt->bind_param("i", $id);

            if ($delete_stmt->execute()) {
                logActivity($_SESSION['user_id'], 'Restore User', "Restored user: {$user['username']}");
        ...
        $error = "User not found in recycle bin!";
    }
}
...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    if ($result->num_rows == 1) {
        ...
        if ($delete_stmt->execute()) {
            logActivity($_SESSION['user_id'], 'Permanent Delete User', "Permanently deleted user: {$user['username']}");
...
// Get all recycled users
$sql = "SELECT *, DATEDIFF(NOW(), deleted_at) as days_in_bin FROM users_recycle ORDER BY deleted_at DESC";
$result = $conn->query($sql);
$recycled_users = $result->fetch_all(MYSQLI_ASSOC);

logActivity($_SESSION['user_id'], 'View Recycle Bin', 'Viewed users recycle bin');
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Recycle Bin - Users - Admin Panel</title>
...
<body data-theme="light">
            ...
            <div class="content">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h2><i class="fas fa-trash me-2"></i>Recycle Bin - Users</h2>
                ...
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="fas fa-trash me-2"></i>Deleted Users</h5>
                        <small class="text-muted">Items are automatically deleted after 10 days</small>
                    </div>
                    <div class="card-body">
                        <?php if (empty($recycled_users)): ?>
                        ...
                        <?php else: ?>
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        <?php foreach ($recycled_users as $index => $user): ?>
                                            <tr class="<?php echo $user['days_in_bin'] >= 7 ? 'table-warning' : ''; ?>">
